...
module LavinMQ
  ...
  describe "constant-encoding differential vs Deque oracle (deterministic, multi-column)" do
    it "matches per-column Deque logs over a deterministic sequence (wrap + materialize + compact)" do
      orig_cap = Config.instance.stats_log_size
      orig_int = Config.instance.stats_interval
      begin
        Config.instance.stats_interval = 5000
        interval = 5.0

        [1, 2, 3, 6, 120].each do |cap|
          Config.instance.stats_log_size = cap
          saw_buffered = 0
          compactions = 0
          steps_total = 0

          (1_u32..24_u32).each do |stream|
            rng = Random::PCG32.new(stream.to_u64)
            p = MultiProbe.new
            ...
            250.times do |step|
              da = (rng.rand(3).zero? ? 0_u64 : rng.rand(200).to_u64 + 1) if rng.rand(10).zero?
              db = (rng.rand(3).zero? ? 0_u64 : rng.rand(200).to_u64 + 1) if rng.rand(10).zero?
              dc = (rng.rand(3).zero? ? 0_u64 : rng.rand(200).to_u64 + 1) if rng.rand(10).zero?
              mv = rng.rand(50).to_u32 if rng.rand(10).zero?
              nv = rng.rand(50).to_u32 if rng.rand(10).zero?
        ...
        Config.instance.stats_log_size = orig_cap
        Config.instance.stats_interval = orig_int
      end
    end
  end
end
